...
@Component
public class FlatDAO implements DAO {  //  DAO для квартир
    ...
    // количество объявлений по всем квартирам  (по фильтру френда/продажа):
    private PreparedStatement numberOfAdsForSaleOrRent;
    // порции короткой инфы по квартирам:
    ...
    public FlatDAO() {
        ...
        try {
            ...
            allObjectsForSaleOrRent = conn.prepareStatement("select id,address,price,type,photo1,forsale from flat where hide = 0 and forsale = ?");
            ...
            portionOfObjects = conn.prepareStatement("select id,address,price,type,photo1,forsale from flat where hide = 0 and forsale = ? limit ?,?");
            numberOfAdsForSaleOrRent = conn.prepareStatement("select count(*) from flat where hide = 0 and forsale = ?");
...
////////////////// АДМИНСКАЯ ЧАСТЬ //////////////////////////////////////////
    ...
    public ArrayList<Flat> getAllAds() {
        ...
        try {
            ResultSet rs = allObjects.executeQuery("select * from flat");
            while (rs.next()) {
                int id = rs.getInt("id");
                boolean forSale = rs.getBoolean("forsale");
                int roomsNumber = rs.getInt("roomsnumber");
