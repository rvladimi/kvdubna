...
@Component
public class Flat3DAO implements DAO {  //  DAO для квартир
    ...
    public Flat3DAO() {
        ...
        try {
            ...
            portionOfObjects = conn.prepareStatement("select id,address,price,type,photo1,forsale from flat where hide = 0 and forsale = ? and roomsnumber = 3 limit ?,?");
            numberOfAdsForSaleOrRent = conn.prepareStatement("select count(*) from flat where hide = 0 and forsale = ? and roomsnumber = 3");
            allObjectsForSaleOrRent = conn.prepareStatement("select id,address,price,type,photo1,forsale from flat where hide = 0 and roomsnumber = 3 and forsale = ?");
...
public ArrayList<Realty> getAllAdsForSaleOrRent(boolean forSale) {
    ...
    public ArrayList<Flat> getAllAds() {
        ...
        try {
            ResultSet rs = allObjects.executeQuery("select * from flat where roomsnumber = 3");
            while (rs.next()) {
                int id = rs.getInt("id");
                boolean forSale = rs.getBoolean("forsale");
                int roomsNumber = rs.getInt("roomsnumber");
