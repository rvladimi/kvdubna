...
@Component
public class Flat4DAO implements DAO {  //  DAO для квартир
    ...
    // количество объявлений по 4+ комнатным квартирам  (по фильтру френда/продажа):
    private PreparedStatement numberOfAdsForSaleOrRent;
    // порции короткой инфы для квартир с 4+ комнатами:
    ...
    public Flat4DAO() {
        ...
        try {
            ...
            allObjectsForSaleOrRent = conn.prepareStatement("select id,address,price,type,photo1,forsale from flat where hide = 0 and roomsnumber >=4  and forsale = ?");
            ...
            portionOfObjects = conn.prepareStatement("select id,address,price,type,photo1,forsale from flat where hide = 0 and forsale = ? and roomsnumber >= 4 limit ?,?");
            numberOfAdsForSaleOrRent = conn.prepareStatement("select count(*) from flat where hide = 0 and forsale = ? and roomsnumber >= 4");
...
public ArrayList<Realty> getAllAdsForSaleOrRent(boolean forSale) {
    ...
    public ArrayList<Flat> getAllAds() {
        ...
        try {
            ResultSet rs = allObjects.executeQuery("select * from flat where roomsnumber >= 4");
            while (rs.next()) {
                int id = rs.getInt("id");
                boolean forSale = rs.getBoolean("forsale");
                int roomsNumber = rs.getInt("roomsnumber");
