...
public class AdminLandController {
    ...
    public ModelAndView panel(HttpServletRequest request, ModelAndView modelAndView) {
        userIPAndDate(request,"AdminLandController");
...
// "Показать список объектов", клик на кнопку "изменить":
            if (adminAction.equals("изменить")) {
                ...
                Land land = landDAO.getLand(id);
                modelAndView.addObject("land", land);
                modelAndView.setViewName("adminLandChange");
            }
// В панели администратора внутри страницы, которая открывается по клику
...
// "Показать список объектов", далее "изменить". В ней клик на кнопку "сохранить изменения"
    @RequestMapping("/landSaveChanges")
...
//  либо из формы (если пользователь менял фото), либо из базы данных (если не менял):
        ...
        int id = Integer.parseInt(request.getParameter("id"));
        Land object = landDAO.getLand(id);
...
//  c:if в adminLandChange.jsp. Создаем объект File, у которого путь
 ...
 */
        ...
        landDAO.update(forSale, address, price, area, electricity, water,
...
// "Открыть форму для заполнения", клик на кнопку "ДОБАВИТЬ ОБЪЕКТ В БАЗУ":
    ...
    public ModelAndView insert(HttpServletRequest request,
        ...
        if (slandarea.isEmpty() || slandarea.length() == 0) {
            area = 0;
        } else {
            area = Double.parseDouble(slandarea);
        }
        String electricity = request.getParameter("electricity");
        String water = request.getParameter("water");
        String gas = request.getParameter("gas");
        ...
        String description = request.getParameter("description");
        boolean hide = false;
        String type = "Дача/участок";

        landDAO.add(forSale, address, price, area, electricity, water, gas, sewerage, description, photo1Name,
