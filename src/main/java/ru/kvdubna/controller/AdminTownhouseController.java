...
import ru.kvdubna.model.Townhouse;
...
@Controller
public class AdminTownhouseController {
    @Autowired
    private TownhouseDAO townhouseDAO;

    @RequestMapping("/admintownhouse")
    public ModelAndView panel(HttpServletRequest request, ModelAndView modelAndView) {
        userIPAndDate(request,"AdminTownhouseController");
...
// "Показать список объектов", клик на кнопку "изменить":
            if (adminAction.equals("изменить")) {
                ...
                Townhouse townhouse = townhouseDAO.getTownhouse(id);
                modelAndView.addObject("townhouse", townhouse);
                modelAndView.setViewName("adminTownhouseChange");
            }
// В панели администратора внутри страницы, которая открывается по клику
// "Показать список объектов", клик на кнопку "удалить":
            if (adminAction.equals("удалить")) {
                int id = Integer.parseInt(request.getParameter("id"));
                townhouseDAO.delete(id);
...
// "Показать список объектов", далее "изменить". В ней клик на кнопку "сохранить изменения"
    @RequestMapping("/townhouseSaveChanges")
...
//  либо из формы (если пользователь менял фото), либо из базы данных (если не менял):
        ...
        int id = Integer.parseInt(request.getParameter("id"));
        Townhouse object = townhouseDAO.getTownhouse(id);
...
//  c:if в adminTownhouseChange.jsp. Создаем объект File, у которого путь
 ...
 */
        ...
        townhouseDAO.update(forSale, address, price,  fullarea, livingarea, kitchenarea, roomsarea, description,
                photo1Name, photo2Name, photo3Name, photo4Name, photo5Name, photo6Name, photo7Name, photo8Name,
                photo9Name, photo10Name, id);
...
// "Открыть форму для заполнения", клик на кнопку "ДОБАВИТЬ ОБЪЕКТ В БАЗУ":
    @RequestMapping("/townhouseInsert")
    public ModelAndView insert(HttpServletRequest request,
        ...
        String description = request.getParameter("description");
        hide = false;
        String type = "Таунхаус";

        townhouseDAO.add(forSale, address, price, fullarea, livingarea, kitchenarea, roomsarea, description,
                photo1Name, photo2Name, photo3Name, photo4Name, photo5Name, photo6Name, photo7Name, photo8Name,
                photo9Name, photo10Name, hide, type);
