...
import ru.kvdubna.model.Cottage;
...
// "Показать список объектов", клик на кнопку "изменить":
            if (adminAction.equals("изменить")) {
                ...
                Cottage cottage = cottageDAO.getCottage(id);
                modelAndView.addObject("cottage", cottage);
                modelAndView.setViewName("adminCottageChange");
            }
// В панели администратора внутри страницы, которая открывается по клику
// "Показать список объектов", клик на кнопку "удалить":
            if (adminAction.equals("удалить")) {
                int id = Integer.parseInt(request.getParameter("id"));
                cottageDAO.delete(id);
...
// "Показать список объектов", далее "изменить". В ней клик на кнопку "сохранить изменения"
    @RequestMapping("/cottageSaveChanges")
...
//  либо из формы (если пользователь менял фото), либо из базы данных (если не менял):
        ...
        int id = Integer.parseInt(request.getParameter("id"));
        Cottage object = cottageDAO.getCottage(id);
...
//  c:if в adminCottageChange.jsp. Создаем объект File, у которого путь
 ...
 */
        ...
        String shomearea = request.getParameter("homeArea");
        if (shomearea == null || shomearea.length() == 0) {
            homeArea = 0;
        } else {
            homeArea = Double.parseDouble(shomearea);
        }
        String slandarea = request.getParameter("landArea");
        if (slandarea == null || slandarea.length() == 0) {
            landArea = 0;
        } else {
            landArea = Double.parseDouble(slandarea);
        }
        String electricity = request.getParameter("electricity");
        String water = request.getParameter("water");
        String gas = request.getParameter("gas");
        String sewerage = request.getParameter("sewerage");
        String description = request.getParameter("description");

        cottageDAO.update(forSale, address, price, homeArea, landArea, electricity, water,
                gas, sewerage, description, photo1Name, photo2Name, photo3Name, photo4Name,
                photo5Name, photo6Name, photo7Name, photo8Name, photo9Name, photo10Name, id);
        modelAndView.addObject("cottageList", cottageDAO.getAllAds());
        modelAndView.setViewName("adminCottageList");
...
// "Открыть форму для заполнения", клик на кнопку "ДОБАВИТЬ ОБЪЕКТ В БАЗУ":
    ...
    public ModelAndView insert(HttpServletRequest request,
        ...
        cottageDAO.add(forSale, address, price, homeArea, landArea, electricity, water, gas, sewerage, description, photo1Name,
                photo2Name, photo3Name, photo4Name, photo5Name, photo6Name, photo7Name, photo8Name, photo9Name,
                photo10Name, hide, type);
