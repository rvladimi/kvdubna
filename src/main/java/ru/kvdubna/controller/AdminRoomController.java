...
public class AdminRoomController {
    ...
    public ModelAndView panel(HttpServletRequest request, ModelAndView modelAndView) {
        userIPAndDate(request,"AdminRoomController");
...
// "Показать список объектов", клик на кнопку "изменить":
            if (adminAction.equals("изменить")) {
                ...
                modelAndView.setViewName("adminRoomChange");
            }
// В панели администратора внутри страницы, которая открывается по клику
...
// "Показать список объектов", далее "изменить". В ней клик на кнопку "сохранить изменения"
    @RequestMapping("/roomSaveChanges")
...
//  либо из формы (если пользователь менял фото), либо из базы данных (если не менял):
        ...
        int id = Integer.parseInt(request.getParameter("id"));
        Room object = roomDAO.getRoom(id);
...
//  c:if в adminRoomChange.jsp. Создаем объект File, у которого путь
 ...
 */
        ...
        String sroomArea = request.getParameter("roomArea");
        if (sroomArea.length()==0) {
            roomArea = 0;
        } else {
            roomArea = Double.parseDouble(sroomArea);
        }
        ...
        roomDAO.update(forSale, roomsNumber, address, price, fullArea, livingArea, kitchenArea, roomArea, floor, floors,
...
// "Открыть форму для заполнения", клик на кнопку "ДОБАВИТЬ ОБЪЕКТ В БАЗУ":
    ...
    public ModelAndView insert(HttpServletRequest request,
        ...
        roomDAO.add(forSale, roomsNumber, address, price, fullArea, livingArea, kitchenArea, roomArea, floor, floors,
                balcony, buildingType, description, photo1Name, photo2Name, photo3Name, photo4Name, photo5Name,
