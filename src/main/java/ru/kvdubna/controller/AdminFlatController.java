...
// "Показать список объектов", клик на кнопку "изменить":
            if (adminAction.equals("изменить")) {
                ...
                Flat flat = flatDAO.getFlat(id);
                modelAndView.addObject("flat", flat);
                modelAndView.setViewName("adminFlatChange");
            }
// В панели администратора внутри страницы, которая открывается по клику
...
// "Показать список объектов", далее "изменить". В ней клик на кнопку "сохранить изменения"
    @RequestMapping("/flatSaveChanges")
...
//  либо из формы (если пользователь менял фото), либо из базы данных (если не менял):
        ...
        int id = Integer.parseInt(request.getParameter("id"));
        Flat object = flatDAO.getFlat(id);
...
//  c:if в adminFlatChange.jsp. Создаем объект File, у которого путь
 ...
 */
        ...
        String sroomsNumber = request.getParameter("roomsNumber");
        if (sroomsNumber == null || sroomsNumber.length()==0) {
            roomsNumber = 0;
        } else {
            roomsNumber = Integer.parseInt(sroomsNumber);
        }
        String address = request.getParameter("address");
        String price = request.getParameter("price");
        String sfullArea = request.getParameter("fullArea");
        if (sfullArea.length()==0) {
            fullArea = 0;
        } else {
            fullArea = Double.parseDouble(sfullArea);
        }
        String slivingArea = request.getParameter("livingArea");
        if (slivingArea.length()==0) {
            livingArea = 0;
        } else {
            livingArea = Double.parseDouble(slivingArea);
        }
        String skitchenArea = request.getParameter("kitchenArea");
        if (skitchenArea.length()==0) {
            kitchenArea = 0;
        } else {
            kitchenArea = Double.parseDouble(skitchenArea);
        }
        String roomsArea = request.getParameter("roomsArea");
        ...
        String sfloors = request.getParameter("floors");
        if (sfloors.length()==0) {
            floors = 0;
        } else {
            floors = Integer.parseInt(sfloors);
        }
        String balcony = request.getParameter("balcony");
        String buildingType = request.getParameter("buildingType");
        String description = request.getParameter("description");

        flatDAO.update(forSale, roomsNumber, address, price, fullArea, livingArea, kitchenArea, roomsArea, floor, floors,
                balcony, buildingType, description, photo1Name, photo2Name, photo3Name, photo4Name, photo5Name,
                photo6Name, photo7Name, photo8Name, photo9Name, photo10Name, id);
...
// "Открыть форму для заполнения", клик на кнопку "ДОБАВИТЬ ОБЪЕКТ В БАЗУ":
    ...
    public ModelAndView insert(HttpServletRequest request,
        ...
        boolean hide = false;
        if(roomsNumber >= 4) {
            type = "Квартира 4+ комн.";
        }
        switch (roomsNumber) {
            case 1:
                type = "Квартира 1-комн.";
                break;
            case 2:
                type = "Квартира 2-комн.";
                break;
            case 3:
                type = "Квартира 3-комн.";
                break;
            case 4:
                type = "Квартира 4+ комн.";
                break;
        ...
        flatDAO.add(forSale, roomsNumber, address, price, fullArea, livingArea, kitchenArea, roomsArea, floor, floors,
                balcony, buildingType, description, photo1Name, photo2Name, photo3Name, photo4Name, photo5Name,
                photo6Name, photo7Name, photo8Name, photo9Name, photo10Name, hide, type);
