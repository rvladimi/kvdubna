...
<c:forEach var="land" items="${landList}">
    <table width="1200" style="background : #FFFDD0;" width="100%" border="0" cellspacing="10" cellpadding="0">
        <tr>
            <td colspan="4">
                    ${land.forSale ? "Продажа" : "Аренда"},
                     Адрес: <b>${land.address}</b>,
                     ${land.hide ? "Объявление снято" : "Объявление активно"}

            </td>
        </tr>
        <tr>
            <form action="/adminland" method="GET">
                <td width="9%">
            ...
            <form action="/adminland" method="GET">
                <td width="13%">
                    <input type="submit" class="select2" style="cursor: pointer;" name="adminAction"
                           value="${land.hide ? "активировать" : "снять"}">
                    <input type="hidden" name="id" value="${land.id}">
                </td>
            </form>
            <td></td>
