...
<table width="1200" border="0" cellspacing="0" cellpadding="10" onmousedown="return false">
    ...
    <tr>
        <td colspan="3" valign="bottom">
            <h2>Cписок объектов коммерческой недвижимости:</h2>
        </td>
    </tr>
</table>
...
<c:forEach var="commercial" items="${commercialList}">
    <table width="1200" style="background : #FFFDD0;" width="100%" border="0" cellspacing="10" cellpadding="0">
        <tr>
            <td colspan="4">
                    ${commercial.forSale ? "Продажа" : "Аренда"},
                     Адрес: <b>${commercial.address}</b>,
                     ${commercial.hide ? "Объявление снято" : "Объявление активно"}

            </td>
        </tr>
        <tr>
            <form action="admincommercial" method="GET">
                <td width="9%">
            ...
            <form action="admincommercial" method="GET">
                <td width="13%">
                    <input type="submit" class="select2" style="cursor: pointer;" name="adminAction"
                           value="${commercial.hide ? "активировать" : "снять"}">
                    <input type="hidden" name="id" value="${commercial.id}">
                </td>
            </form>
            <td></td>
