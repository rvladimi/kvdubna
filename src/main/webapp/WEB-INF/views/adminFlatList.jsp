...
<c:forEach var="flat" items="${flatList}">
    <table width="1200" style="background : #FFFDD0;" width="100%" border="0" cellspacing="10" cellpadding="0">
        <tr>
            <td colspan="4">
                    ${flat.forSale ? "Продажа" : "Аренда"},
                     Адрес: <b>${flat.address}</b>,
                     ${flat.hide ? "Объявление снято" : "Объявление активно"}
            </td>
        </tr>
        <tr>
            <form action="adminflat" method="GET">
                <td width="9%">
            ...
            <form action="adminflat" method="GET">
                <td width="13%">
                    <input type="submit" class="select2" style="cursor: pointer;" name="adminAction"
                           value="${flat.hide ? "активировать" : "снять"}">
                    <input type="hidden" name="id" value="${flat.id}">
                </td>
            </form>
            <td></td>
